...
## 2026.4.0: Duplicate mscorlib - Bug Fix
Features:
- [Feature-2026-5] Added support for Unity UI Toolkit (UIElements) custom types in the compatibility component. Custom VisualElement/UXML types are now automatically skipped from obfuscation to keep UXML bindings valid.

Improvements:
- [Imprv-2026-7] The Anti-Tampering component only supported files up to 2GB (Int32) in size. This has been increased by using Int64 [thanks Alex].

Bug Fixes:
- [Bug-2026-9] Fixed an issue where obfuscated .NET Standard assemblies could get an additional mscorlib reference, causing UnityLinker/IL2CPP builds to fail on DOTS/Unity.Entities projects [thanks Mikael].

## 2026.3.0: Addressable Binary - Update
Features:
- [Feature-2026-4] Added support for the Addressable binary catalog, next to json catalog.

Improvements:
- [Imprv-2026-6] Optimized the anti-tampering component to use a more efficient checksum algorithm. Depending on the runtime system, hardware acceleration is not supported, which caused the integrity checks to take a long time on app/game startup.

## 2026.2.2: Maintenance - Update
Improvements:
- [Imprv-2026-5] Refactored the stack trace deobfuscation window tool. It now autoloads the settings mapping and has been optimized to deobfuscate stack traces more effectively [thanks Daztrax].

Bug Fixes:
- [Bug-2026-8] Fixed an issue where enabling control flow obfuscation caused the obfuscated build to fail. The cause was incorrectly resolved try/catch blocks.
- [Bug-2026-7] Enabling public property and field obfuscation caused issues in some IL2CPP builds due to named parameters that IL2CPP could not resolve [thanks Lemior].

## 2026.2.1: Addressable Shield - Update
Features:
- [Feature-2026-3] Added support for Addressable Shield (Unity 6+). This allows you to obfuscate addressable bundles that are encrypted using Addressable Shield.
